...
#last updated version 2022-07-04
# test on 
...
Usage() {
	...
	echo -e "\toptional arguments:\n " \
          ...
          "\t\t-a  (--account)           = account for slurm job submissions [CURRENT: rrg-grouleau-ac]\n" \
          "\t\t-c  (--config)            = config file [CURRENT: \"$(realpath $(dirname $0))/configs/segrun.config.ini\"]\n" \
...
if [[ $MODE =~ [1-3] && ! -s $VCF ]]; then echo "ERROR: missing mandatory option for steps 1-3: -v (--vcf) empty, not specified or does not exist"; FOUND_ERROR=1; fi
if [[ $MODE =~ [1-3] && ! -s $PED ]]; then echo "ERROR: missing mandatory option for steps 1-3: -p (--ped) empty, not specified or does not exist"; FOUND_ERROR=1; fi

if (( $FOUND_ERROR )); then echo "Please check options and try again"; exit 42; fi
if [ $MODE == 'ALL' ]; then  MODE0=`echo {1..3}`; fi 

# set default variables
ACCOUNT=${ACCOUNT:-rrg-grouleau-ac}
CONFIG_FILE=${CONFIG_FILE:-$PIPELINE_HOME/configs/segrun.config.ini}
...
#    touch $OUTPUT_DIR/expected.done.files.txt
#  else
#    rm -f  $OUTPUT_DIR/expected.done.files.txt
#    touch $OUTPUT_DIR/expected.done.files.txt
#  fi
...
#export EXPECTED_DONE_FILES=$OUTPUT_DIR/expected.done.files.txt
...
if [[  ${MODE0[@]} =~ 1 ]] ; then
  ...
  step_1="$QUEUE -A $ACCOUNT  \
    ...
    --time=${WALLTIME} \
    --job-name $STEP \
    --export OUTPUT_DIR=${OUTPUT_DIR},PIPELINE_HOME=${PIPELINE_HOME},BASEFOLDER=${BASEFOLDER},VCF=${VCF},GRCH=${GRCH} \
...
if [[  ${MODE0[@]}  =~  2 ]]  &&  [[  ${MODE0[@]} =~ 1 ]] ; then
  ...
  step_2="$QUEUE -A $ACCOUNT  \
    ...
    --job-name $STEP \
    $DEPEND_step_1 \
    --export OUTPUT_DIR=${OUTPUT_DIR},PIPELINE_HOME=${PIPELINE_HOME},BASEFOLDER=${BASEFOLDER},VCF=${VCF},PED=${PED},NCOL=${NCOL},SPARKMEM=${SPARKMEM} \
    --output $OUTPUT_DIR/logs/slurm/%x.o%j \
