...
##### seg withOUT CSQ just one phenotype
def segrun_family_wise_withoutCSQ_genotype_sole(mt, ped, outfolder,hl):
    ###
    ...
    for i in fam_list:
        ...
        # Affected memeber
        # unaffected
        cmd_all=f'cd {outfolder}/temp ;paste -d\'\t\' locus_alleles.csv glb_aff.csv fam_{i}.csv > fam_{i}_all.csv'
        os.system(cmd_all)
    ...
    for i in fam_list:
        ...
        os.system(cmd_rem0)
    cmd_rmall=f'cd {outfolder}/temp ;rm locus_alleles.csv glb_aff.csv fam_*.csv'
    os.system(cmd_rmall)
